...
export default function AIWorkoutGenerator() {
  ...
  const steps = [
    ...
    "Designing custom 7-day split...",
    "Finalizing exercise selection..."
  ];
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-[fade-in-up_0.6s_ease-out_forwards]">
      <button 
        onClick={() => navigate(-1)}
        className="flex items-center gap-2 text-sm font-medium text-zinc-400 hover:text-emerald-400 transition-colors disabled:opacity-50"
      ...
      <div className="relative rounded-[2rem] border border-zinc-800/60 bg-zinc-900/40 p-10 text-center shadow-2xl shadow-emerald-500/5 backdrop-blur-xl overflow-hidden">
        {/* Background glow effect */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-96 h-96 bg-emerald-500/10 blur-[100px] pointer-events-none" />
        ...
        <div className="relative z-10 mx-auto mb-8 flex h-20 w-20 items-center justify-center rounded-2xl bg-zinc-950 border border-zinc-800/50 shadow-inner">
          <Bot className={`h-10 w-10 ${isGenerating ? 'text-emerald-400 animate-pulse drop-shadow-[0_0_15px_rgba(16,185,129,0.8)]' : 'text-zinc-400'}`} />
        </div>

        <h1 className="relative z-10 text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-500 mb-4">AI Routine Architect</h1>
